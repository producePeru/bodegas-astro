...
<template>
  <section class="max-w-6xl mx-auto px-4 text-center">
    <h1
      class="font-display text-2xl md:text-3xl text-[#00a6db] tracking-wide"
    >
      Encuentra tu tienda
    </h1>
    ...
    <div id="buscador" class="mt-6 max-w-sm mx-auto">
      ...
      <p
        v-if="!cargando && !error && visibles.length === 0"
        class="mt-4 text-sm text-[#00a6db]"
      >
  ...
  <section class="w-full px-4 md:px-8 py-10 md:py-14">
    ...
    >
      <div
        v-for="n in 8"
        :key="n"
        class="bg-white border border-stone-200/60 rounded-lg overflow-hidden"
      >
        <div
          class="h-28 md:h-72 bg-gradient-to-br from-stone-50 to-stone-100 flex items-center justify-center p-4"
        >
          <div
            class="w-14 h-14 md:w-24 md:h-24 rounded-full bg-stone-200/70 animate-pulse"
          ></div>
        </div>
        <div
          class="h-10 md:h-11 px-2 border-t border-stone-100 flex items-center justify-center"
        >
    ...
    >
      <a
        v-for="tienda in visibles"
        :key="tienda.id"
        :href="`#/marca/${tienda.id}`"
      ...
      >
        ...
        >
          ...
          <span
            v-else
            class="font-body font-semibold text-center leading-tight text-sm md:text-base"
            >{{ tienda.nombre }}</span
          >
        </div>
        <span
      ...
      <p
        v-if="!cargando && visibles.length === 0"
        class="col-span-full text-center text-stone-400"
      >
